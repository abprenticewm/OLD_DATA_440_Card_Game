...
CHUNK_SIZE = 10_000   # number of decks per file picked 10k to match other method and because it it smaller chunk that could be replicated
OUT_DIR = "decks_chunks"  # folder to save decks
BYTEORDER = "big" #The most significant byte comes first

# do a check NUM_DECKS must be divisible by CHUNK_SIZE and figure out how many chuncks we need
...
def generate_chunk(chunk_index: int, out_dir: str = OUT_DIR):
    ...
    path = os.path.join(out_dir, filename) #file path 

    # Generate decks 
    gen_start = time.perf_counter() #start geenration time 
    #creates all decks - 52 bits per deck, converted to bytes 
    decks = [
        rng.getrandbits(DECK_SIZE_BITS).to_bytes(BYTES_PER_DECK, byteorder=BYTEORDER)
    ...
    with open(path, "wb") as f: #wb means w: overwite if already exists and b: in binary 
        f.writelines(decks) #writes everything into files 
...
def generate_chunks(start_chunk: int = 0, num_chunks: int = NUM_CHUNKS, out_dir: str = OUT_DIR):
    ...
    """
    created = [] #files paths of everything created 
    #times start at zero 
    total_gen_time = 0.0
...
def read_decks(created_files):
    ...
    for filename in created_files:
        with open(filename, "rb") as f: #open in read binary mode, close write after!!!!!!
            while (chunk := f.read(BYTES_PER_DECK)): #read each chunk 
                deck_int = int.from_bytes(chunk, "big") #raw bytes must go back to an integer to be read 
                _ = format(deck_int, f"0{DECK_SIZE_BITS}b") #into a 52 lenth binary string for easy searching later 

    runtime = time.perf_counter() - start_time
    return runtime #runtime only needed metric to return 
